# Cesium
> Cesium三维 GIS 可视化学习，不断拓展记录。大多为外网网站，官网也为外网，dddd

## 参考文档
* [Cesium官方教程](https://cesium.com/learn/)
* [Cesium英文API官方文档](https://cesium.com/learn/cesiumjs/ref-doc/)
* [Cesium沙盒示例（常用）(官方的一些示例程序,多看看能做啥特效)](https://sandcastle.cesium.com/)
* [Cesium中文API文档（常用）](http://cesium.xin/cesium/cn/Documentation1.62/)
* [Cesium中文网（常用）](http://cesium.xin/)
* [免费3d模型下载](https://sketchfab.com/feed)
* [3d模型下载](https://polyhaven.com/zh)
* [geojson数据下载阿里云Datav](https://datav.aliyun.com/portal/school/atlas/area_selector)

## 开始使用Cesium

前提条件：[点击前往官网下载](https://cesium.com/downloads)
- PS:主要复制`Build下面的Cesium`文件夹到项目中会引入到此`Cesium`文件夹。
- PS:国内可能会很慢，可如下百度网盘下载：
- 链接: https://pan.baidu.com/s/1sOrQx-2JJsQeYEH3-dUvKQ?pwd=ib74 提取码: ib74

1. 引入 cesium.js，因为此文件定义了 Cesium 对象，我们需引用
```javascript
<script src="./Cesium/Cesium.js"></script>
```
2. 引入 widgets.css，为了能使用Cesium 各个可视化控件
```css
@import url(./Cesium/Widgets/widgets.css);
```
3. 在 HTML 的 body 中我们创建一个 div，用来作为三维地球的容器
```html
<div id="cesiumContainer"></div>
```
4. 初始化 CesiumViewer 实例
```javascript
...
- [详细了解](http://cesium.xin/wordpress/archives/58.html)

**代码示例：**(仅剩一个地图，去除了地图上多余界面)
```html
<!DOCTYPE html>
<html lang="en">
<head>
...
<script>
    window.onload = function () {
        // 1. Geocoder:查找位置工具，查找到之后会将镜头对准找到的地址，默认使用bing地图
        // 2. Home Button :视角返回初始位置.
        // 3. Scene Mode Picker:选择视角的模式，有三种:3D，2D，哥伦布视图(CV)4. Base Layer Picker:图层选择器，选择要显示的地图服务和地形服务
        // 5.Navigation Help Button :导航帮助按钮，显示默认的地图控制帮助
        // 6.Animation:动画器件，控制视图动画的播放速度
        // 7.Timeline :时间线,指示当前时间，并允许用户跳到特定的时间8. Credits Display :版权显示，显示数据归属，必选
        // 9.Fullscreen Button :全屏按钮
        const viewer = new Cesium.Viewer("cesiumContainer", {
            geocoder: false,
            homeButton: false,
...
```
## 绘制各种形状
>通过entity实现绘制功能,详细配置查阅文档

* [entity中文API了解](http://cesium.xin/cesium/cn/Documentation1.62/)
...
<script>
    window.onload = function () {
        ...
        // 绘制正方体形状
        let orangeBox = viewer.entities.add({
            name: 'Red box with black outline',
            position: Cesium.Cartesian3.fromDegrees(-106.8, 35.0, 300000.0),
            box: {
        ...
        let point = viewer.entities.add({
            ...
            point: {
                pixelSize: 50,
                //自定义颜色 Cesium.Color.fromCssColorString
                color: Cesium.Color.fromCssColorString('#fc4a14'),
            }
        });
        ...
        let sphereEntity = viewer.entities.add({
            name: 'Sphere',
            position: Cesium.Cartesian3.fromDegrees(-107.2, 40.0, 0),
            ellipsoid: {
        ...
        // 绘制椭球
        let ellipsoidEntity = viewer.entities.add({
            name: 'Ellipsoid',
            position: Cesium.Cartesian3.fromDegrees(-107.4, 40.0, 0),
            ellipsoid: {
                radii: new Cesium.Cartesian3(20000, 30000, 40000),
        ...
        let wallEntity = viewer.entities.add({
            ...
            wall: {
                ...
                maximumHeights: [20000, 20000, 20000, 20000, 20000],
        ...
        let ellipseEntity = viewer.entities.add({
            name: 'Ellipse',
            position: Cesium.Cartesian3.fromDegrees(-108.0, 39.0, 0),
            ellipse: {
        ...
        viewer.zoomTo([orangeBox, point, cylinderEntity, sphereEntity, ellipsoidEntity, wallEntity, ellipseEntity]);
    }
</script>
</body>
</html>
...
[官网教程](https://cesium.com/learn/cesiumjs-learn/cesiumjs-flight-tracker/)

在这里说下重点交代，具体的可以上官网翻译为中文查看每一步教程。
1. [cesium ion注册](https://ion.cesium.com/signup)
2. [查看令牌Default Token](https://ion.cesium.com/tokens?page=1) (即点击Access Tokens查看右侧即可看到)
3. [3d模型下载](https://sketchfab.com/feed)
4. 下载完3d模型后到[选择 Add Data=>Add Files=>选择刚刚下载的导出为gltf`整个压缩包`进行上传=>选择 3D 模型（转换为 glTF）](https://ion.cesium.com/tokens?page=1)
5. 处理后找到Assets ID，还有代码生成


**完整代码如下，主要是修改2点：**
* 这里替换为你的default token =》Cesium.Ion.defaultAccessToken = '' 
* 这里替换为你的3d模型的your_asset_id=》const airplaneUri = await Cesium.IonResource.fromAssetId(your_asset_id);
```html
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <script type="module">
    ...
    //1-这里替换为你的default token!!!
    Cesium.Ion.defaultAccessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIyZmIzOTBkNC0zZjgyLTQzN2YtYjVmYi0wZjQ1NDZiNGI2NWIiLCJpZCI6MjAzNTMwLCJpYXQiOjE3MTEwMTEyNTh9.I0fI2a3Txe52OHuG6bTrnhHY7Z-BZGyvuqMLomzFgFE';
    ...
    */
    ...
    async function loadModel() {
      // Load the glTF model from Cesium ion.
        // 2-这里替换为你的your_asset_id!!!  就是上传的3d模型 Assets Id
